...
export default function ChatView({documents, onBack}) {
    ...
    const sendMessage = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`http://localhost:7070/api/document/chat?query=${input}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                const aiMessage = {id: Date.now(), role: 'ai', content: data.message, avatar: '🤖'};
                setMessages(prev => [...prev, aiMessage])
            } else {
                const errorMessage = {id: Date.now(), role: 'ai', content: 'Failed to get ai response !'};
                    ...
                    {messages.map((msg, index) => (
                        ...
                        >
                                <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl shadow-lg ${
                                    ...
                                    <p>{msg.content}</p>
                                    {loading && (index === messages.length-1) && <span className="text-white/80 text-sm font-medium">AI is typing...</span>}
                                </div>
                        </motion.div>
                    ))}
                </AnimatePresence>
